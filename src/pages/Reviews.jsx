...
const Reviews = () => {
    ...
    useEffect(() => {
        const fetchReview = async () => {
            const { data } = await axios(`${import.meta.env.VITE_API_LINK}/reviews`)
            setReviews(data)
        }
        fetchReview()
    }, [])
    return (
        <div className="mt-16">
            <div className='mb-10'>
                <h2 className='text-4xl font-bold text-center uppercase '>Feedback </h2>
                <h2 className='text-xl font-bold text-center '>What Our Users Says About Us!</h2>
            </div>
            ...
            <Swiper navigation={true} modules={[Navigation]} className="mySwiper">
                {
                    reviews.map(item => <SwiperSlide key={item._id}>
                        <div className="flex flex-col items-center gap-4 mx-24">
                            {/* <ReactStarsRating value={item.rating} /> */}
                                <Rating
                                style={{ maxWidth: 180 }} 
                                value={item.rating} />
                            ...
                            <p className="w-5/6 text-center">{item.details}</p>
                            <h3 className="text-[#CD9003] uppercase text-3xl">{item.name}</h3>
                        </div>
